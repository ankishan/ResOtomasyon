...
public class CollectionPagerAdapter extends FragmentPagerAdapter {
    ...
    public Fragment getItem(int i) {
        ...
        for (int j = 0; j < lstDepartmanlar.size(); j++) {
            if (i == j) {
                for (int k=0;k<masaPlanIsmi.length;k++)
                {
                    if(lstDepartmanlar.get(i).DepartmanEkrani.contentEquals(masaPlanIsmi[k]))
                    {
                        masaIsimleri = new ArrayList<String>();
                        for (MasaDizayn masaDizayn : lstMasaDizayn) {
                            if (masaDizayn.MasaEkraniAdi.contentEquals(lstDepartmanlar.get(i).DepartmanEkrani)) {
